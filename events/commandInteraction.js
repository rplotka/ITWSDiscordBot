...
module.exports = {
  ...
  async execute(interaction) {
    ...
    logger.info(`Command found: ${interaction.commandName}, isModeratorOnly: ${command.isModeratorOnly || false}`);
    ...
    if (command.isModeratorOnly) {
      try {
        ...
        if (!interaction.member || !interaction.member.permissions) {
          logger.warn(`Permission check failed: member=${!!interaction.member}, permissions=${!!interaction.member?.permissions}`);
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({
              content: '❌ Only moderators can run that command! (No permissions object)',
              ephemeral: true,
            });
          }
          return;
        ...
        logger.info(`Permission check for ${interaction.user.tag}: ADMINISTRATOR=${hasAdmin}, MANAGE_GUILD=${hasManageGuild}, hasPermission=${hasPermission}`);
        ...
        logger.error(`PermissionFlagsBits object: ${typeof PermissionFlagsBits}, Administrator: ${typeof PermissionFlagsBits?.Administrator}`);
        ...
        if (!interaction.replied && !interaction.deferred) {
          try {
            ...
            logger.error(`Failed to send permission error reply: ${replyError.message}`);
          }
        }
        return;
      }
