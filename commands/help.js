module.exports = {
    ...
    execute(message, member, args) {
        ...
        if (args.length === 0) {
            ...
            const messageLines = [
                '**Commands**',
                ...commands.mapValues(command => `\`${command.name}\` - ${command.description}`).array()
            ];
            ...
            const usageLines = Object.entries(helpCommand.usages)
                .map(([usage, description]) => `\`${usage}\` - ${description}`);

            const capitalizedCommandName = helpCommand.name.charAt(0).toUpperCase() + helpCommand.name.slice(1);
